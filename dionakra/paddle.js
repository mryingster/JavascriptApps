class Paddle {
    ...
    collide() {
        ...
        for (let enemy of enemies) {
            if (!enemy.ignore) {
                if (enemy.pos.y + enemy.height > this.pos.y) {
                    let d = Math.abs(enemy.pos.x + (enemy.width/2) - (this.pos.x + this.width));
                    let md = enemy.radius + (this.width / 3);
                    if (d < md) {
                        console.log(d, md);
                        enemy.hit();
                    }
    ...
    render_autorpilot_timer() {
        let time = this.autopiloting / 1000;
        ...
        this.ctx.fillText(time.toFixed(2), sizes.frame.left * 2, sizes.arena.bottom - sizes.frame.left);
    }
...
class Laser {
    ...
    move(ms) {
        ...
        for (let enemy of enemies) {
            if (!enemy.ignore) {
                // Get hypot from enemy that we treat like circles
                let d1 = Math.hypot(enemy.pos.x + (enemy.width/2) - left_center_pos,  enemy.pos.y + (enemy.height/2) - this.pos.y);
                let d2 = Math.hypot(enemy.pos.x + (enemy.width/2) - right_center_pos, enemy.pos.y + (enemy.height/2) - this.pos.y);
                if (d1 < enemy.radius || d2 < enemy.radius){
                    enemy.hit();
                    this.remove = true;
                }
            }
